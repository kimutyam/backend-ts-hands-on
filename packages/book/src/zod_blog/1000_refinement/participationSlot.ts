...
import { z } from 'zod';
import { fromZodReturnTypeDefault } from '../700_refinement/resultBuilder';
import type { SlatChange } from './slotChange';
...
const increase =
  (change: SlatChange) =>
  (p: ParticipationSlot): Result<ParticipationSlot, z.ZodError<ParticipationSlotInput>> =>
    safeBuild(p + change);
