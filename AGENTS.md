...
This repo is a Yarn workspaces monorepo. Package code lives under `packages/` with focused modules such as `packages/fp`, `packages/fp-zod`, `packages/pure`, and `packages/hono-drizzle`. Each package keeps source code in `src/` and tests in `test/` or `__tests__/` directories (example: `packages/hono-drizzle/src/app/useCase/__tests__/getCart.test.ts`). Shared tooling and configs sit at the root (for example `eslint.config.mjs`, `tsconfig.json`, `.prettierrc.mjs`).
...
## Testing Guidelines
Jest is used in `fp`, `fp-zod`, `pure`, and `nest-sample`; Vitest is used in and `hono-drizzle`. Run coverage where available using `pnpm --filter <pkg> test:cov`. For `nest-sample`, end-to-end tests live under `packages/nest-sample/test` and run with `pnpm --filter nest-sample test:e2e`.
